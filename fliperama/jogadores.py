...
#Disciplina: Pensamento Computacional, Aloritimo e Programacao
# (2026-PCAP)
# Aula: 22 - MeuApp v2.0: o cadastro de jogadores
# Autor: Maria Julia Pereira Majer
# Revisado: Aula 23 - validacao de campo vazio e documento
...
def cadastrar(jogadores):
    '''
    Pergunta apelido e nome e acrescenta um jogador ao cadastri.
    ...
    apelido = ler_texto('Apelido (sem espaco)').lower()
...
def listar(jogadores):
    titulo('JOGADORE CADASTRADOS')

    if len(jogadores) ==0:
        print('Nenhum jogador cadatredo ainda.')
    else:
...
def buscar(jogadores, apelido):
    ...
    Retorno:
        int - posicao do jogadore na lista, ou -1 se nao acahr
        '''
...
def alterar(jogadores):
    ...
    apelido = input('Apelido de quem vai mudar de nome: ').strip().lowe()
